...
public class AggregationGraph extends JPanel implements Runnable {
    	   ...
    	   if((adjList.get(node) != null) && (!adjList.get(node).isEmpty())) {
	    	   for (int adj: adjList.get(node)) {
		    	   if(adjList.get(node).contains(adj)) {
			           int x1 = graphPoints.get(node).x;
			           int y1 = graphPoints.get(node).y;
			           int x2 = graphPoints.get(adj).x;
			           int y2 = graphPoints.get(adj).y;
	...
	public void run() {
		String graphName= "Aggregation Graph (minimum weight k tree)";
